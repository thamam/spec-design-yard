...
* **Browser-Storage Opt-Out:** For sketching without filesystem writes, the picker offers "Use browser storage instead" — specs then auto-save to the browser's `localStorage` (with an in-memory fallback), and this is the one place the built-in demo spec appears — a first launch opens the same blank slate a fresh project gets. The opt-out is remembered until you pick a project again.
