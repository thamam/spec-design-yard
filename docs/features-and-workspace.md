...
* **Project Badge & Picker:** The header always shows the active project folder's name (hover for the full path). Clicking it opens the picker: the full path, a field to switch to another project directory (with create-if-missing), and a recent-projects list. Switching reloads the workspace against the new folder's files; a tab still open on the previous project has its saves refused. In local mode the picker APIs only answer loopback requests; in opt-in remote mode they require a signed-in session.
* **Opt-in remote access:** `npm run dev:remote` / `spec-yard --remote` plus Tailscale Serve lets a phone on the same tailnet open the workspace after a token login. Default launches stay loopback and unauthenticated. See `SECURITY.md`.
...
* **Browser-Storage Opt-Out:** For sketching without filesystem writes, the picker offers "Use browser storage instead" — specs then auto-save to the browser's `localStorage` (with an in-memory fallback). Opt-out keeps the current spec (no demo surprise, no white reload). A later standalone visit with an empty browser cache opens the same labeled blank slate a fresh project gets. The opt-out is remembered until you pick a project again.
