...
# Every scenario runs on BASE_PORT + 0..5, so guarding only BASE_PORT let
...
KNOWN_SCENARIOS="file-mode first-run standalone editor-ergonomics focus-disclosure arrow-anchors"
if [ -n "$ONLY" ]; then
  MATCHED=0
...
# --- 6. arrow anchors: edge-pinned arrow bindings in a real Excalidraw ---
ARROW_CLIENT="$TMP/arrow-anchors-repo"
mkdir -p "$ARROW_CLIENT"
SCENARIO_ENV=("SPEC_YARD_E2E_CLIENT=$ARROW_CLIENT")
run_scenario "arrow-anchors" "$((BASE_PORT + 5))" "scripts/e2e-arrow-anchors.py" \
  "SPEC_YARD_PROJECT_DIR=$ARROW_CLIENT"
