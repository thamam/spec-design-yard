...
- `npm run dev` — dev server on loopback (`127.0.0.1`), unauthenticated
- `npm run dev:remote` — same bind, opt-in remote token + login (Tailscale Serve)
- `npm run start` — production server on loopback (`127.0.0.1`)
...
- `npm run test:e2e` — real-browser scenarios (`scripts/run-e2e.sh`; add a scenario name to run just one: `file-mode`, `first-run`, `standalone`, `editor-ergonomics`, `focus-disclosure`, `arrow-anchors`). Needs `playwright` + `playwright install chromium`. Each scenario gets its own dev server on 3109-3114, its own project folders, and its own `SPEC_YARD_CONFIG_DIR` under a temp root — never point it at your own registry or at port 3000. Failing scenarios leave their screenshots in `.e2e-failures/`.
...
- Project selection is GUI-first: header picker (`components/workspace/project-picker.tsx` → `pages/api/project.ts`; loopback-Host + JSON-content-type guarded, absolute/exists/writable validation). `lib/server-project-config.ts` keeps session state on globalThis (plain module state would be duplicated per API-route bundle) and is server-only (imports node `crypto`/`fs`) — never import it from client code. Every switch re-mints a project "epoch"; client PUTs echo it (`?epoch=`) and a stale one 409s (`project-switched`), so a tab on the old project can't clobber the new one. `bin/spec-yard <dir>` retargets a running instance through the same API after probing `GET /api/auth/session` (bearer token is attached only when that reports `remote:true`; `--remote` refuses a local-mode occupant). Opt-in remote mode (`SPEC_YARD_REMOTE=1` / `--remote`) adds `lib/server-auth.ts` (local token under the config dir, session cookie + generation revoke on logout, CSRF header) and Host allowlisting for this machine's Tailscale name; default launches stay loopback with no auth. Never import `server-auth.ts` from client code.
- Fresh projects (`{found:false}`) open with the labeled blank `FRESH_PROJECT_SPEC`, never the unused `INITIAL_SPEC` demo, and nothing is autosaved until the user edits. So does a first run. Browser-storage opt-out **keeps the current spec** (no demo surprise, no full-page white reload). A later standalone visit with an empty cache opens a labeled blank slate. The demo must never be written into a project folder uninvited. The editor/canvas stay inert until hydration **and** a first-run folder/opt-out decision is in front of the user.
