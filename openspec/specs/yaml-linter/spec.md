# yaml-linter Specification

## Purpose
The YAML static analyzer (`yaml-linter`) parses and validates system specifications on every user keystroke, establishing active quality and security diagnostics for system-level settings, components, connections, and system flow anti-patterns.
...
#### Scenario: Unrecognized system key
- GIVEN a system spec with key `unrecognizedKey: value` in top-level system block
- WHEN analyzed by `lintSpec`
- THEN a diagnostic with code `unrecognized-system-key` is returned listing alphabetical keys: `components, metadata, name`.
