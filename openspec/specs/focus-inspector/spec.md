# focus-inspector Specification

## Purpose

The Focus tab: a progressive inspector for the selected component (or
global system settings when nothing is selected), so ordinary architecture
work is not buried under every schema field and a compiled YAML dump.
